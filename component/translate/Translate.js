...
const translateText = async (text, fromLanguage, toLanguage) => {
  try {
    const response = await axios.post('http://192.168.124.16:5000/translate', {
...
const generateSpeech = async (text, language) => {
  try {
    const response = await axios.post('http://192.168.124.16:5001/tts', {
      text: text,
      language: language,
    ...
    if (response.data.status === 'success' && response.data.audio_url) {
      const audioUrl = `http://192.168.124.16:5001${response.data.audio_url}`;
...
const Translate = () => {
  ...
  const transcribeAudio = async (uri) => {
    ...
    try {
      const response = await axios.post('http://192.168.124.16:5002/transcribe', formData, {
        headers: {
